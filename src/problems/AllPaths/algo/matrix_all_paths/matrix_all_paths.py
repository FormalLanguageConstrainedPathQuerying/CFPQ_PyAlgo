...
class LibBuilder:
    def __init__(self):
        if not os.path.isfile(PATH_TO_SO.joinpath("libAllPaths.so")):
            raise Exception(
                "Please run the command 'make' in src/problems/AllPaths/algo/matrix_all_paths/impl"
            )
...
class MatrixAllAlgo:
    def prepare(self, graph: Path, grammar: Path):
        ...
        self.grammar = self.lib.grammar_new(
            str(grammar.with_suffix(".cnf")).encode("utf-8")
        )
    ...
    def getPaths(self, v_start, v_finish, nonterminal, max_len):
        return self.lib.getpaths(
            self.grammar, v_start, v_finish, nonterminal.encode("utf-8"), max_len
        )
